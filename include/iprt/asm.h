 ...
 */
#define ASMAtomicCmpXchgU8(pf , fNew, fOld) _InterlockedCompareExchange8((volatile char *)pf, (char)fNew, (char)fOld);
#if 0
#if RT_INLINE_ASM_EXTERNAL || !RT_INLINE_ASM_USES_INTRIN
...
DECLINLINE(bool) ASMAtomicCmpXchgU8(volatile uint8_t *pu8, const uint8_t u8New, uint8_t u8Old)
{
    uint8_t u8Ret;
#if RT_INLINE_ASM_USES_INTRIN
    u8Ret = _InterlockedExchange8((volatile char *)pu8, u8New);   
    return true;            
#else
 ...
 */
DECLINLINE(int) ASMAtomicCmpXchgS8(volatile int8_t *pi8, const int8_t i8New, const int8_t i8Old)
{
 ...
 */
DECLINLINE(int) ASMAtomicCmpXchgBool(volatile bool *pf, const bool fNew, const bool fOld)
{
