...
findExtFunction( const crOpenGLInterface *crInterface, const char *funcName )
{
#ifdef WINDOWS
	if (crInterface->wglGetProcAddress)
		return (SPUGenericFunction) crInterface->wglGetProcAddress( funcName );
	else
	...
	if (f)
		return f;
#if !defined(DARWIN)
	else if (crInterface->glXGetProcAddressARB)
		return crInterface->glXGetProcAddressARB( (const GLubyte *) funcName );
#endif
	else
		return NULL;
#endif
 ...
 */
int
crLoadOpenGL( crOpenGLInterface *crInterface, SPUNamedFunctionTable table[] )
{
...
for fun in useful_wgl_functions:
	print ( '\tcrInterface->%s = (%sFunc_t) crDLLGetNoError( glDll, "%s" );' % (fun,fun,fun) )

print ( '#elif defined(DARWIN)' )
print ( '# ifndef VBOX_WITH_COCOA_QT' )
for fun in useful_agl_functions:
	print ( '\tcrInterface->%s = (%sFunc_t) crDLLGetNoError( aglDll, "%s" );' % (fun,fun,fun) )
print ( '# endif' )

for fun in useful_cgl_functions:
	print ( '\tcrInterface->%s = (%sFunc_t) crDLLGetNoError( cglDll, "%s" );' % (fun, fun,fun) )
...
print ( """
	...
	entry->name = NULL;
	entry->fn = NULL;
	return (int) ( entry - table );  /* number of entries filled */
}
 ...
 */
int
crLoadOpenGLExtensions( const crOpenGLInterface *crInterface, SPUNamedFunctionTable table[] )
{
	struct extfunc {
		const char *funcName;
...
#ifdef WINDOWS
	if (crInterface->wglGetProcAddress == NULL)
		crWarning("Unable to find wglGetProcAddress() in system GL library");
#elif !defined(DARWIN)
	if (crInterface->glXGetProcAddressARB == NULL)
		crWarning("Unable to find glXGetProcAddressARB() in system GL library");
#endif

	for (func = functions; func->funcName; func++) {
		SPUGenericFunction f = findExtFunction(crInterface, func->funcName);
		if (!f && func->aliasName) {
			f = findExtFunction(crInterface, func->aliasName);
		}
		if (!f) {
			f = func->nopFunction;
		}
