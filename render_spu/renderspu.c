...
#ifdef RT_OS_WINDOWS
static TCHAR * makeTCharName(char * title)
{
    size_t convertedChars = 0;
    size_t titleSize = crStrlen(title);
    TCHAR * safeTitle = crAlloc(crStrlen(title) * sizeof(TCHAR));
    mbstowcs_s(&convertedChars, safeTitle, titleSize, title, _TRUNCATE);
    return safeTitle;
}
#else
static TCHAR * makeTCharName(char * title)
{
    char * retString = crAlloc(crStrlen(title) * sizeof(TCHAR));
    crStrcpy(retString, title);
    return retString;
}
#endif
static void freeWindowTitles(WindowInfo * window)
{
    if(window->title)
        crFree(window->title);
    if(window->tTitle)
        crFree(window->tTitle);
    window->title = NULL;
    window->tTitle = NULL;
}
...
WindowInfo* renderspuWinCreate(GLint visBits, const char * dpyName, GLint id)
{
    ...
    if (!renderspuWinInit(window, dpyName, visBits, id))
    {
        crWarning("renderspuWinInit failed");
        freeWindowTitles(window);
        crFree(window);
        return NULL;
    }
...
WindowInfo* renderspuGetDummyWindow(GLint visBits)
{
    ...
    if (!window)
    {
        window = renderspuWinCreate(visBits, "Dummy", -1);
        if (!window)
        {
...
GLboolean renderspuWinInitWithVisual( WindowInfo *window, VisualInfo *visual, GLboolean showIt, GLint id )
{
    ...
    /* Set window->title, replacing %i with the window ID number */
    {
        ...
        if (s) {
            ...
            k = sprintf_s(window->title + i, sizeof(window->title)-i, "%d", window->BltInfo.Base.id);
            CRASSERT(k < 10);
    ...
    //Make a TCHAR version of the title that is safe for Windows calls.
    window->tTitle = makeTCharName(window->title);
...
GLboolean renderspuWinInit(WindowInfo *pWindow, const char *dpyName, GLint visBits, GLint id)
{
    ...
    */
    /* Have GLX/WGL/AGL create the window */
    if(id == CR_RENDER_DEFAULT_WINDOW_ID )
        showIt = 0;
    if (!renderspuWinInitWithVisual( pWindow, visual, showIt, id ))
    {
        crWarning( "Render SPU: Couldn't create a window, renderspu_SystemCreateWindow failed" );
        return GL_FALSE;
    }
...
GLint renderspuWindowCreateEx( const char *dpyName, GLint visBits, GLint id )
{
    ...
    /* Allocate WindowInfo */
    window = renderspuWinCreate(visBits, dpyName, id);
...
#else
        sprintf_s(tempStr,  sizeof(tempStr), "Chromium (%s)", (char *) render_spu.ws.glGetString(GL_RENDERER));
#endif
