...
static void set_window_geometry( RenderSPU *render_spu, const char *response )
{
    int x, y, w, h;
    CRASSERT(response[0] == '[');
    sscanf_s( response, "[ %d, %d, %d, %d ]", &x, &y, &w, &h );
...
{
    sscanf_s( response, "%d", &(render_spu->fullscreen) );
}

static void set_on_top( RenderSPU *render_spu, const char *response )
{
    sscanf_s( response, "%d", &(render_spu->ontop) );
}
...
{
    sscanf_s( response, "%d", &(render_spu->gather_userbuf_size) );
}
...
void renderspuSetVBoxConfiguration( RenderSPU *render_spu )
{
    ...
    set_window_geometry(render_spu, "[0, 0, 800, 600]");
