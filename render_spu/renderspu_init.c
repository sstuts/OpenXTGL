...
#ifdef RT_OS_WINDOWS
DECLEXPORT(void) renderspuStopWindowThreadProc()
{
    if(!PostThreadMessage(render_spu.dwWinThreadId, WM_VBOX_RENDERSPU_DESTROY_THREAD, 0, 0)) {
        crError("Failed to halt the WndProc thread %d", render_spu.dwWinThreadId);
    }
    WaitForSingleObject(render_spu.hWinThreadReadyEvent, 1000);
}
static DWORD WINAPI renderSPUWindowThreadProc(void* unused)
{
    ...
    while( (bRet = GetMessage( &msg, 0, 0, 0 )) != 0)
    {
        ...
        else
        {
            ...
            else if(msg.message == WM_VBOX_RENDERSPU_DESTROY_THREAD) {
                break;
            }
            else
            {
