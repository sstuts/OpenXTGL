...
{
	const int num_data = (int) crPackNumData(src);
	const int num_opcode = (int) crPackNumOpcodes(src);
    int res;
...
crPackAppendBoundedBuffer( CR_PACKER_CONTEXT_ARGDECL const CRPackBuffer *src, const CRrecti *bounds )
{
	...
	const int num_opcodes = (int) crPackNumOpcodes(src);
	const int length = (int ) (src->data_current - src->opcode_current - 1);
