    ...
    if apiutil.IsPointer(arg_type):
        ...
        if is_swapped:
            ...
            if arg_type == "GLdouble" or arg_type == "GLclampd":
                ...
                retval = "\tWRITE_DATA( %d, %s, %s );" % (offset, arg_type, arg_name)
    if retval == 9:
        print ( "no retval for %s %s" % (arg_name, arg_type), file=stderr)
        assert 0
    return retval
...
def PrintFunc( func_name, params, is_swapped, can_have_pointers ):
    ...
    if is_swapped:
        print ( 'DLLDATA( void ) PACK_APIENTRY crPack%sSWAP( %s )' % (func_name, apiutil.MakeDeclarationStringWithContext('CR_PACKER_CONTEXT', params)) )
    else:
        print ( 'DLLDATA( void ) PACK_APIENTRY crPack%s( %s )' % (func_name, apiutil.MakeDeclarationStringWithContext('CR_PACKER_CONTEXT', params)) )
    print ( '{' )
