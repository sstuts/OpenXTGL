...
for func_name in keys:
	if ("pack" in apiutil.ChromiumProps(func_name) or
		...
		print ( 'DLLDATA( void ) PACK_APIENTRY crPack%s( %s );' % (func_name, apiutil.MakeDeclarationStringWithContext('CR_PACKER_CONTEXT', args)) )
		print ( 'DLLDATA( void ) PACK_APIENTRY crPack%sSWAP( %s );' % (func_name, apiutil.MakeDeclarationStringWithContext('CR_PACKER_CONTEXT', args)) )
...
for func_name in keys:
	if (func_name[0:6] == "Vertex" and
		...
		args = apiutil.Parameters(func_name)
		print ( 'DLLDATA( void ) PACK_APIENTRY crPack%sBBOX(%s);' % (func_name, apiutil.MakeDeclarationString(args)) )
		print ( 'DLLDATA( void ) PACK_APIENTRY crPack%sBBOX_COUNT(%s);' % (func_name, apiutil.MakeDeclarationString(args)) )
		print ( 'DLLDATA( void ) PACK_APIENTRY crPack%sBBOXSWAP(%s);' % (func_name, apiutil.MakeDeclarationString(args)) )
		print ( 'DLLDATA( void ) PACK_APIENTRY crPack%sBBOX_COUNTSWAP(%s);' % (func_name, apiutil.MakeDeclarationString(args)) )
