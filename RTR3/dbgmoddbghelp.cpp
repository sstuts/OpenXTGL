...
static BOOL CALLBACK rtDbgModDbgHelpCopyLineNumberCallback(PSRCCODEINFOW pLineInfo, PVOID pvUser)
{
    ...
    size_t cbLen  = (RTUtf16Len((PRTUTF16)pLineInfo->FileName) + 1) * sizeof(RTUTF16);
    if (   !pArgs->pwszPrev
        || memcmp(pArgs->pwszPrev, pLineInfo->FileName, cbLen) )
    {
        ...
        pArgs->pszPrev = NULL;
        int rc = RTUtf16ToUtf8((PCRTUTF16)pLineInfo->FileName, &pArgs->pszPrev);
        if (RT_FAILURE(rc))
        {
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (SymInitialize(hFake, NULL /*SearchPath*/, FALSE /*fInvalidProcess*/))
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    uint64_t uModAddr = SymLoadModuleExW(hFake, NULL /*hFile*/, (PCWSTR) pwszDbgFile, NULL /*pszModName*/,
