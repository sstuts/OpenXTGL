...
RTDECL(bool) RTEnvExistsUtf8(const char *pszVar)
{
    ...
    bool fRet = _wgetenv((const wchar_t *)pwszVar) != NULL;
    RTUtf16Free(pwszVar);
    return fRet;
}
...
RTDECL(int) RTEnvGetUtf8(const char *pszVar, char *pszValue, size_t cbValue, size_t *pcchActual)
{
    ...
    PCRTUTF16 pwszValue = (PCRTUTF16)_wgetenv((const wchar_t *)pwszVar);
    RTUtf16Free(pwszVar);
    if (pwszValue)
    {
        if (cbValue)
...
RTDECL(int) RTEnvPutUtf8(const char *pszVarEqualValue)
{
    ...
    if (RT_SUCCESS(rc))
    {
        if (!_wputenv((const wchar_t *)pwszVarEqualValue))
            rc = VINF_SUCCESS;
        else
            ...
            if (pwszTmp)
            {
                ...
                if (RT_SUCCESS(rc))
                {
                    ...
                    *pwszTmpValue++ = '=';
                    ...
                    if (RT_SUCCESS(rc))
                    {
                        if (!_wputenv((const wchar_t *)pwszTmp))
                            rc = VINF_SUCCESS;
                        else
                            rc = RTErrConvertFromErrno(errno);
                    }
