                                      ...
                                      PCRTDWARFABBREV pAbbrev, PRTDWARFDIE pParent)
{
    ...
    if (pDie)
    {
        ...
        pDie->iAllocator = (uint16_t) iAllocator;
#endif
...
#ifdef RTDBGMODDWARF_WITH_MEM_CACHE
    uint32_t ii = RT_ELEMENTS(pThis->aDieAllocators);
    while (ii-- > 0)
    {
        RTMemCacheDestroy(pThis->aDieAllocators[ii].hMemCache);
        pThis->aDieAllocators[ii].hMemCache = NIL_RTMEMCACHE;
    }
#endif
