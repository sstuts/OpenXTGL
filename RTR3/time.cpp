/* $Id: time.cpp 56978 2015-07-18 18:55:25Z vboxsync $ */
/** @file
 * IPRT - Time.
 */
...
static PRTTIME rtTimeNormalizeInternal(PRTTIME pTime)
{
    ...
    if (!pTime->u16YearDay)
    {
        ...
        for (;;)
        {
            ...
            if (pTime->u8MonthDay <= cDaysInMonth)
                break;
            pTime->u8MonthDay -= (uint8_t) cDaysInMonth;
            if (pTime->u8Month != 12)
                pTime->u8Month++;
            else
            {
    ...
    pTime->u8Second = (uint8_t) uSecond;
    pTime->u8Minute = (uint8_t) uMinute;
    pTime->u8Hour = (uint8_t) uHour;
