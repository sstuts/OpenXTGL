...
RTDECL(int) RTPathUserHome(char *pszPath, size_t cchPath)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            HRESULT hrc = pfnSHGetFolderPathW(0, CSIDL_PROFILE, NULL, SHGFP_TYPE_CURRENT, (LPWSTR)wszPath);
    ...
    DWORD   dwAttr;
    if (    !fValidFolderPath
        ||  (dwAttr = GetFileAttributesW((LPCWSTR)&wszPath[0])) == INVALID_FILE_ATTRIBUTES
        ||  !(dwAttr & FILE_ATTRIBUTE_DIRECTORY))
    {
        /*
         * Fall back to Windows specific environment variables. HOME is not used.
         */
        if (    !GetEnvironmentVariableW(L"USERPROFILE",(LPWSTR) &wszPath[0], RTPATH_MAX)
            ||  (dwAttr = GetFileAttributesW((LPCWSTR)&wszPath[0])) == INVALID_FILE_ATTRIBUTES
            ||  !(dwAttr & FILE_ATTRIBUTE_DIRECTORY))
        {
            /* %HOMEDRIVE%%HOMEPATH% */
            if (!GetEnvironmentVariableW(L"HOMEDRIVE", (LPWSTR)&wszPath[0], RTPATH_MAX))
                return VERR_PATH_NOT_FOUND;
            size_t const cwc = RTUtf16Len(&wszPath[0]);
            if (    !GetEnvironmentVariableW(L"HOMEPATH", (LPWSTR)&wszPath[cwc], RTPATH_MAX - (DWORD)cwc)
                ||  (dwAttr = GetFileAttributesW((LPWSTR)&wszPath[0])) == INVALID_FILE_ATTRIBUTES
                ||  !(dwAttr & FILE_ATTRIBUTE_DIRECTORY))
                return VERR_PATH_NOT_FOUND;
        }
    }
...
RTDECL(int) RTPathUserDocuments(char *pszPath, size_t cchPath)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        {
            RTUTF16 wszPath[RTPATH_MAX];
            HRESULT hrc = pfnSHGetFolderPathW(0, CSIDL_PERSONAL, NULL, SHGFP_TYPE_CURRENT,(LPWSTR) wszPath);
            if (   hrc == S_OK     /* Found */
...
RTR3DECL(int) RTPathQueryInfoEx(const char *pszPath, PRTFSOBJINFO pObjInfo, RTFSOBJATTRADD enmAdditionalAttribs, uint32_t fFlags)
{
    ...
    if (RT_FAILURE(rc))
        return rc;
    if (!GetFileAttributesExW((LPCWSTR)pwszPath, GetFileExInfoStandard, &Data))
    {
        /* Fallback to FindFileFirst in case of sharing violation. */
        if (GetLastError() == ERROR_SHARING_VIOLATION)
        {
            WIN32_FIND_DATAW FindData;
            HANDLE hDir = FindFirstFileW((LPCWSTR)pwszPath, &FindData);
    ...
    {
        HANDLE hFinal = CreateFileW((LPCWSTR)pwszPath,
                                    GENERIC_READ,
                               ...
                               PCRTTIMESPEC pChangeTime, PCRTTIMESPEC pBirthTime, uint32_t fFlags)
{
    ...
    if (RT_SUCCESS(rc))
    {
        HANDLE hFile;
        if (fFlags & RTPATH_F_FOLLOW_LINK)
            hFile = CreateFileW((LPCWSTR)pwszPath,
...
DECLHIDDEN(int) rtPathWin32MoveRename(const char *pszSrc, const char *pszDst, uint32_t fFlags, RTFMODE fFileType)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (fFileType)
            {
                DWORD dwAttr = GetFileAttributesW((LPCWSTR)pwszSrc);
                if (dwAttr == INVALID_FILE_ATTRIBUTES)
                    rc = RTErrConvertFromWin32(GetLastError());
            ...
            if (RT_SUCCESS(rc))
            {
                if (MoveFileExW((LPCWSTR)pwszSrc, (LPCWSTR) pwszDst, fFlags))
                    rc = VINF_SUCCESS;
                else
                {
                    DWORD Err = GetLastError();
...
RTDECL(bool) RTPathExistsEx(const char *pszPath, uint32_t fFlags)
{
    ...
    if (RT_SUCCESS(rc))
    {
        dwAttr = GetFileAttributesW((LPCWSTR)pwszPath);
        RTUtf16Free(pwszPath);
    }
    else
...
RTDECL(int) RTPathGetCurrent(char *pszPath, size_t cchPath)
{
    ...
    if (GetCurrentDirectoryW(RTPATH_MAX,(LPWSTR) wszCurPath))
    {
        RTUTF16 wszFullPath[RTPATH_MAX];
        if (GetFullPathNameW((LPCWSTR)wszCurPath, RTPATH_MAX, (LPWSTR)wszFullPath, NULL))
...
RTDECL(int) RTPathSetCurrent(const char *pszPath)
{
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (!SetCurrentDirectoryW((LPCWSTR)pwszPath))
            rc = RTErrConvertFromWin32(GetLastError());
...
RTDECL(int) RTPathGetCurrentOnDrive(char chDrive, char *pszPath, size_t cbPath)
{
    ...
    int rc;
    RTUTF16 wszFullPath[RTPATH_MAX];
    if (GetFullPathNameW((LPWSTR)wszInput, RTPATH_MAX,(LPWSTR) wszFullPath, NULL))
