/* $Id: process-win.cpp 56290 2015-06-09 14:01:31Z vboxsync $ */
/** @file
 ...
 */
static bool rtProcWinFindTokenByProcessAndPsApi(const wchar_t * const *papszNames, PSID pSid, PHANDLE phToken)
{
    bool fFound = false;
    ...
    if (RT_SUCCESS(rc))
    {
         ...
         * than RTPATH_MAX.
         */
        DWORD cbProcName  = RTPATH_MAX * sizeof(wchar_t);
        wchar_t *pszProcName = (wchar_t *)RTMemTmpAlloc(cbProcName);
        if (pszProcName)
        {
            for (size_t i = 0; papszNames[i] && !fFound; i++)
            {
                ...
                for (DWORD iPid = 0; iPid < cPids && !fFound; iPid++)
                {
                    ...
                    {
                        *pszProcName = '\0';
                        DWORD cbRet = pfnGetModuleBaseName(hProc, 0 /*hModule = exe */,(LPTSTR) pszProcName, cbProcName);
                        if (   cbRet > 0
                            && _wcsicmp(pszProcName, papszNames[i]) == 0
 ...
 *                          caller closes it on success.
 */
static bool rtProcWinFindTokenByProcess(const wchar_t * const *papszNames, PSID pSid, PHANDLE phToken)
{
        ...
        if (hSnap != INVALID_HANDLE_VALUE)
        {
            ...
            for (size_t i = 0; papszNames[i] && !fFound; i++)
            {
                ...
                if (pfnProcess32First(hSnap, &procEntry))
                {
                    do
                    {
                        if (   _wcsicmp(procEntry.szExeFile, papszNames[i]) == 0
...
static int rtProcWinUserLogon(PRTUTF16 pwszUser, PRTUTF16 pwszPassword, PRTUTF16 pwszDomain, HANDLE *phToken)
{
    ...
    BOOL fRc = LogonUserW((LPCWSTR)pwszUser,
                          /*
        ...
        if (fFlags & RTPROC_FLAGS_SERVICE)
        {
                ...
                && cbSid > 0)
            {
                    ...
                    && IsValidSid(pSid))
                {
                    ...
                    static const wchar_t * const s_papszProcNames[] =
                    {
                                  ...
                                  STARTUPINFOW *pStartupInfo, PROCESS_INFORMATION *pProcInfo, uint32_t fFlags)
{
    ...
    if (RT_SUCCESS(rc))
    {
        BOOL fRc = pfnCreateProcessWithLogonW((LPCWSTR) pwszUser,
                                              NULL,                       /* lpDomain*/
            (LPCWSTR) pwszPassword,
...
#ifdef  OXT_OPENGL
static RTDECL(char *) RTEnvDupEx(RTENV Env, const char *pszVar)
{
    /*
    ...
    if (RT_SUCCESS(rc))
    {
        ...
        if (RT_SUCCESS(rc))
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                {
                    if (CreateProcessW((LPCWSTR) pwszExec,
                        (LPWSTR) pwszCmdLine,
                                       NULL,         /* pProcessAttributes */
                                       NULL,         /* pThreadAttributes */
                                       TRUE,         /* fInheritHandles */
                                  ...
                                  size_t *pcbUser)
{
    ...
    if (!GetUserNameW((LPWSTR)&awszUserName[0], &cchUserName))
