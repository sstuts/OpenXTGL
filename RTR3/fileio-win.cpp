/* $Id: fileio-win.cpp 56290 2015-06-09 14:01:31Z vboxsync $ */
/** @file
...
RTR3DECL(int) RTFileOpen(PRTFILE pFile, const char *pszFilename, uint64_t fOpen)
{
    ...
    *pFile = NIL_RTFILE;
    ...
    HANDLE hFile = CreateFileW((LPCWSTR)pwszFilename,
                               dwDesiredAccess,
                               dwShareMode,
                               pSecurityAttributes,
    ...
    if (hFile != INVALID_HANDLE_VALUE)
    {
        ...
        {
            if (!SetFileAttributesW((LPCWSTR)pwszFilename, FILE_ATTRIBUTE_NOT_CONTENT_INDEXED))
...
RTR3DECL(int)  RTFileDelete(const char *pszFilename)
{
    ...
    if (RT_SUCCESS(rc))
    {
        if (!DeleteFileW((LPCWSTR)pwszFilename))
            rc = RTErrConvertFromWin32(GetLastError());
        RTUtf16Free(pwszFilename);
    }
