...
void crServerSetVBoxConfiguration()
{
...
#else
    strcpy_s(response, sizeof(response), "1 0 render");
#endif
    ...
    if (cr_server.vncMode) {
        ...
        strcpy_s(response, sizeof(response), "1 tcpip 1");
    }
...
#else
        sscanf_s(clientlist[i], "%s %d", cr_server.protocol, sizeof(cr_server.protocol), &(newClient->spu_id));
#endif
...
void crServerSetVBoxConfigurationHGCM()
{
    ...
    strcpy_s(cr_server.protocol, sizeof(cr_server.protocol), "vboxhgcm");
