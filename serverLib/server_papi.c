...
void SERVER_DISPATCH_APIENTRY crServerDispatchBarrierCreateCR( GLuint name, GLuint count )
{
...
#if DEBUG_BARRIERS
	sprintf_s( debug_buf, sizeof(debug_buf), "BarrierCreateCR( %d, %d )", name, count );
...
#if DEBUG_BARRIERS
		sprintf_s( debug_buf,  sizeof(debug_buf), "changing count to %d", count );
		cr_server.head_spu->dispatch_table.ChromiumParametervCR( GL_PRINT_STRING_CR, GL_UNSIGNED_BYTE, sizeof(debug_buf), debug_buf );
#endif
	}
...
#if DEBUG_BARRIERS
		sprintf_s( debug_buf, sizeof(debug_buf), "This was a new barrier!" );
		cr_server.head_spu->dispatch_table.ChromiumParametervCR( GL_PRINT_STRING_CR, GL_UNSIGNED_BYTE, sizeof(debug_buf), debug_buf );
#endif
	}
	else
	{
		/* HACK -- this allows everybody to create a barrier, and all
           ...
           match. */
#if DEBUG_BARRIERS
		sprintf_s( debug_buf, sizeof(debug_buf), "I already knew about this barrier." );
		cr_server.head_spu->dispatch_table.ChromiumParametervCR( GL_PRINT_STRING_CR, GL_UNSIGNED_BYTE, sizeof(debug_buf), debug_buf );
#endif
		if ( barrier->count != count )
		{
#if DEBUG_BARRIERS
            sprintf_s(debug_buf, sizeof(debug_buf), "And someone messed up the count!.");
			cr_server.head_spu->dispatch_table.ChromiumParametervCR( GL_PRINT_STRING_CR, GL_UNSIGNED_BYTE, sizeof(debug_buf), debug_buf );
#endif
...
#if DEBUG_BARRIERS
    sprintf_s(debug_buf, sizeof(debug_buf), "BarrierExec( %d )", name);
	cr_server.head_spu->dispatch_table.ChromiumParametervCR( GL_PRINT_STRING_CR, GL_UNSIGNED_BYTE, sizeof(debug_buf), debug_buf );
    sprintf_s(debug_buf, sizeof(debug_buf), "num_waiting = %d", barrier->num_waiting);
	cr_server.head_spu->dispatch_table.ChromiumParametervCR( GL_PRINT_STRING_CR, GL_UNSIGNED_BYTE, sizeof(debug_buf), debug_buf );
#endif
