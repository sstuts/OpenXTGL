...
#define CRSTATE_SET_INT(state, cap) {GLint _i= (GLint)(g->state); diff_api.GetIntegerv(cap, &_i); CRSTATE_SET_CAP(state, _i, "%i");}
#define CRSTATE_SET_SHORT(state, cap) {GLint _i= (GLint)(g->state); diff_api.GetIntegerv(cap, &_i); CRSTATE_SET_CAP(state, (GLushort)_i, "%i");}
...
#define CRSTATE_SET_STENCIL_OP(_idx, _suff) do { \
    ...
    if (CHECKDIRTY(sb->viewport.dirty, negbitID))
    {
        ...
        {
            GLfloat value[2];
            value[0] = (GLfloat)g->viewport.nearClip;
            value[1] = (GLfloat)g->viewport.farClip;
    ...
    if (CHECKDIRTY(sb->line.dirty, negbitID))
    {
        ...
        if (CHECKDIRTY(sb->line.stipple, negbitID))
        {
            ...
            CRSTATE_SET_SHORT(line.pattern, GL_LINE_STIPPLE_PATTERN);
        }
    }
...

        {
            ...
            for (i=0; i<g->limits.maxTextureUnits; ++i)
            {
                if (CHECKDIRTY(sb->point.coordReplacement[i], negbitID))
                {
                    ...
                    CRSTATE_SET_CAP(point.coordReplacement[i], val? GL_TRUE : GL_FALSE, "%i");
                }
            }
