...
#ifdef RT_OS_WINDOWS
static void dumpPixelMap(GLint id, GLsizei width, GLsizei height, GLenum format, GLenum type, const GLvoid * pixels) {
	WCHAR buffer[256];
	...
	//QAD so only allow RGBA GL_UNSIGNED_BYTE
	if(format != GL_RGBA || type != GL_UNSIGNED_BYTE) return;

	data_size = height * width * 4;
	swprintf_s(buffer, sizeof(buffer), L"d:\\Sandbox\\OXT3D\\Pixmaps\\Texture_%d.bmp", id);
	...
	bfh.bfType = 0X4D42;
	headers_size = sizeof(BITMAPINFOHEADER) + sizeof(BITMAPFILEHEADER);
	bfh.bfOffBits = headers_size;
	bfh.bfSize = headers_size + data_size;
	...
	WriteFile(hFile, pixels, bmpInfoHeader.biSizeImage, &dwWritten, NULL);
	CloseHandle(hFile);
}
#endif
void STATE_APIENTRY
...
#endif
#ifdef RT_OS_WINDOWS
	dumpPixelMap(tobj->id, width, height, format, type, pixels);
#endif
}
