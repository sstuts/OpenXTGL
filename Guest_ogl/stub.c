...
extern void __stdcall packspuTurnPrint(int flag);
    ...
    if (winW && winH) {
        ...
        if (stub.trackWindowPos) {
            if (bForceUpdate || winX != pWindow->x || winY != pWindow->y) {
                crDebug("Dispatched WindowPosition (%i) (%d, %d)", pWindow->spuWindow,winX, winY);
#ifdef VBOX_WITH_WDDM
                if (!stub.bRunningUnderWDDM || pWindow->mapped)
#endif
                {
...
LRESULT CALLBACK stubCBWindowMessageHookProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    if (nCode>=0 && pMsgInfo)
    {
        switch (pMsgInfo->message)
        {
            ...
            case WM_NCHITTEST:
            {
                ...
                retVal = CallNextHookEx(stub.hMessageHook, nCode, wParam, lParam);
                //crDebug("%s returning %d for chitest %d", __FUNCTION__, retVal, counter++);
                if(retVal == 0) {
                    //double check this?
                    return HTCLIENT;
                }
            ...
            case WM_KILLFOCUS:
            {
                ...
                crDebug("%s: %s for window %d", __FUNCTION__, pMsgInfo->message == WM_SETFOCUS ? "Set focus " : "Kill focus");
    ...
    retVal =  CallNextHookEx(stub.hMessageHook, nCode, wParam, lParam);
    if(pMsgInfo->message == WM_NCHITTEST)
        crDebug("%s return test result %d", __FUNCTION__, retVal);
    return retVal;
}
