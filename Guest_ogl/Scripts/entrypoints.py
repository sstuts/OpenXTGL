...
def GenerateEntrypoints(hacks = []):
    ...
    for index in range(len(keys)):
        ...
        if func_name in hacks:
            ...
            # the usual path
            print ( "GLAPI %s GLAPIENTRY cr_gl%s( %s );" % (return_type, func_name, apiutil.MakeDeclarationString(params)) )
            print ( "" )
            print ( "GLAPI %s GLAPIENTRY cr_gl%s( %s )" % (return_type, func_name, apiutil.MakeDeclarationString(params)) )
            print ( "{" )
            print ( "\t", )
