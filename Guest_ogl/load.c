...
static void stubCheckWindowState(WindowInfo *window, GLboolean bFlushOnChange)
{
    ...
    if (stub.trackWindowVisibility && window->type == CHROMIUM && window->drawable) {
        ...
        if (mapped != window->mapped) {
            ...
            window->mapped = (GLboolean)mapped;
            bChanged = true;
        }
    }
...
LookupMothershipConfig(const char *procName)
{
    ...
    home = crGetenv("HOME");
    if (home)
        sprintf_s(configPath, sizeof(configPath), "%s/%s", home, CONFIG_LOOKUP_FILE);
    else
...
stubInitLocked(void)
{
    ...
    strcpy_s(response, sizeof(response), "2 0 feedback 1 pack");
...
# endif
        ...
        crNetInit(NULL, NULL);
        ns.name = "vboxhgcm://host:0";
        env = getenv("OXTOGL_SERVER");
        ns.name = crStrdup(env);
        ns.buffer_size = 1024;
        crNetServerConnect(&ns
#if defined(VBOX_WITH_CRHGSMI) && defined(IN_GUEST)
                , NULL
