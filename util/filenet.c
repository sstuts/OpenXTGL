...
crFileReadExact( CRConnection *conn, void *buf, unsigned int len )
{
	...
	while ( len > 0 )
	{
		int num_read = _read( conn->fd, buf, len );
...
{
	int retval = _write( conn->fd, buf, len );
	if ( retval < (int) len )
	{
...
crFileAccept( CRConnection *conn, const char *hostname, unsigned short port )
{
	...
	conn->fd = _open( conn->filename, O_RDONLY | O_BINARY );
	if (conn->fd < 0)
	{
...
crFileDoConnect( CRConnection *conn )
{
	 ...
	 */
#ifdef WINDOWS
	conn->fd =_open( conn->filename, O_CREAT | O_WRONLY | O_BINARY |
									 S_IREAD | S_IWRITE);
#else
