...
#ifdef RT_OS_WINDOWS
static void LogToDebugOutputV(const char *pszFormat, va_list va)
{
    char output[4096];
    TCHAR tString[4096];
    char *ptr = output;
    int n = _vsnprintf(ptr, sizeof(output) - 3, pszFormat, va);
    ptr += n;
#if defined(_DEBUG)
...
static void LogToDebugOutput(const char *pString)
{
#if defined(_DEBUG)
...
static void logMessageV(const char *pszPrefix, const char *pszFormat, va_list va)
{
    ...
    if (RTR3InitIsInitialized())
    {
        va_copy(vaCopy, va);
#ifdef RT_OS_WINDOWS
        LogToDebugOutput(pszPrefix);
        LogToDebugOutputV(pszFormat, vaCopy);
        LogToDebugOutput("\r\n\0");
#else
...
DECLEXPORT(void) crError(const char *pszFormat, ...)
{
    ...
    if (dwLastErr != 0 && crGetenv("CR_WINDOWS_ERRORS") != NULL)
    {
        ...
        FormatMessageA(  FORMAT_MESSAGE_ALLOCATE_BUFFER
                       ...
                       (LPSTR)&pszWindowsMessage, 0, NULL);
...
DECLEXPORT(void) crDebug(const char *pszFormat, ...)
{
    if (RTR3InitIsInitialized())
    {
        ...
        va_start(va, pszFormat);
#ifdef RT_OS_WINDOWS
        logMessageV("crDebug: ", pszFormat, va);
        va_end(va);
#else
...
#ifdef RT_OS_WINDOWS
DECLEXPORT(void) crSetDebugFlag(int flag)
{
    DebugFlag = flag;
    crDebug("%s turning flag %d", __FUNCTION__, flag);
}
DECLEXPORT(void) crDebugFlagged(const char *pszFormat, ...)
{
    if(!DebugFlag) return;
    va_list va;
    va_start(va, pszFormat);
    logMessageV("crDebuggFlagged: ", pszFormat, va);
    va_end(va);
}
#endif
