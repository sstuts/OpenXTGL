...
char *crTCPIPErrorString( int err )
{
    ...
    sprintf_s( buf, sizeof(buf), "err=%d", err );
...
void crCloseSocket( CRSocket sock )
{
    ...
    if (SocketDestroyCallback) {
        SocketDestroyCallback(CR_SOCKET_DESTROY, (int)sock);
    }
...
#define MAX_PORTS 100
    ...
    static SOCKET sockets[MAX_PORTS];
    static int SocketCount = 0;

static void
ClearListeningSocket(int port)
{
    int i;
...
crTCPIPAccept( CRConnection *conn, const char *hostname, unsigned short port )
{
    ...
    if (SocketCreateCallback) {
        SocketCreateCallback(CR_SOCKET_CREATE, (int)conn->tcp_socket);
    }
...
crTCPIPReceiveMessage(CRConnection *conn)
{
    ...
    const SOCKET sock = conn->tcp_socket;
...
crTCPIPDoDisconnect( CRConnection *conn )
{
...
#if 1 /* disabled on 13 Dec 2005 by BrianP - this prevents future client
    ...
    if (none_left && cr_tcpip.server_sock != -1)
    {
        ...
        ClearListeningSocket((int)cr_tcpip.server_sock);
        cr_tcpip.server_sock = -1;
    }
#endif
}
