...
void crA2T(TCHAR * dst,  size_t dstSize, const char * str)
{
    size_t convertedChars = 0;
    mbstowcs_s(&convertedChars, dst, dstSize/4, str, _TRUNCATE);
}
#endif
...
void crBytesToString( char *string, int nstring, void *data, int ndata )
{
	...
	{
		offset += sprintf_s( string + offset, 2, "%02x ", udata[i] );
	}
...
void crWordsToString( char *string, int nstring, void *data, int ndata )
{
	...
	for ( i = 0; i < ndata; i++ ) 
	{
		offset += sprintf_s( string + offset, 8, "%08x ", udata[i] );
	}
...
char **crStrSplit( const char *str, const char *splitstr )
{
	...
	for (i = 0 ; i < num_args ; i++)
	{
		...
		faked_argv[i] = crStrndup( temp, (unsigned int)(end-temp ));
